...
const FAQItem = ({ question, answer, isOpen, onClick, index, icon }) => {
  ...
  return (
    ...
    >
      <motion.button
        onClick={onClick}
        className="w-full py-4 px-4 flex items-center justify-between text-left transition-all duration-300 hover:bg-indigo-50/50 rounded-lg group"
        whileHover={{ scale: 1.01 }}
        whileTap={{ scale: 0.99 }}
      >
        <div className="flex items-center gap-3">
          <div className="p-2 bg-blue-50 rounded-full">{icon}</div>
          <h3 className="text-lg font-medium text-gray-900 pr-8 group-hover:text-blue-500 transition-colors duration-300">
            {question}
          </h3>
        </div>
        <motion.div
        ...
        >
          <BiChevronDown className="w-6 h-6 text-blue-500 group-hover:text-purple-400 transition-colors duration-300" />
        </motion.div>
      </motion.button>
      ...
      <AnimatePresence>
        {isOpen && (
          ...
          >
            <motion.div
              className="p-4 pt-0 text-gray-600 text-base ml-12"
...
const HomeFAQs = () => {
  ...
  return (
    <div ref={sectionRef} className="py-24 px-4 bg-gradient-to-b from-white to-indigo-50 relative overflow-hidden custom-scrollbar-3d">
      <motion.div
      ...
      <motion.div className="max-w-5xl mx-auto relative z-10" style={{ y, opacity, scale }}>
        <motion.div
        ...
        >
          ...
          <h2 className="text-4xl font-bold text-gray-900 mb-4">Healthcare FAQs</h2>
          <motion.div
        ...
        <div className="relative mb-8">
          <motion.button
            onClick={scrollLeft}
            className="absolute left-0 top-1/2 -translate-y-1/2 z-10 bg-white rounded-full shadow-md p-2 hover:bg-gray-100 transition-all duration-300 hover:scale-105"
            aria-label="Scroll left"
            whileHover={{ scale: 1.1 }}
            whileTap={{ scale: 0.95 }}
          >
            <BiChevronLeft className="w-6 h-6 text-blue-500" />
          </motion.button>
          ...
          >
            {categories.map((category, index) => (
              <motion.button
                ...
                className={`
                  whitespace-nowrap px-6 py-2 rounded-full mx-2 transition-all duration-300 snap-start shadow-sm
                  ${
          ...
          <motion.button
            onClick={scrollRight}
            className="absolute right-0 top-1/2 -translate-y-1/2 z-10 bg-white rounded-full shadow-md p-2 hover:bg-gray-100 transition-all duration-300 hover:scale-105"
            aria-label="Scroll right"
            whileHover={{ scale: 1.1 }}
            whileTap={{ scale: 0.95 }}
          >
            <BiChevronRight className="w-6 h-6 text-blue-500" />
          </motion.button>
        </div>
